...
class TestDriver2(unittest.TestCase):
    ...
    def test_test_requests(self):
        ...
        exp = []
        for i in range(0, s):  exp.append(i)
        exp[0]  = 0xFF
        exp[-1] = 0xCF

        res = []
        for i in range(0, s):  res.append(r[i])
...
#        print("EXP: ", exp)
    ...
    def test_should_scan_all_network_parameters(self):
        ...
        s11, s21, s12, s22 = driver.scan(freq, 10,  pocketvna.NetworkParams.ALL,
            on_progress
        )
...
class TestRfMathCalibration(unittest.TestCase):
    ...
    def assertComplexArrayNear(self, c1, cexp, msg):
        ...
        for idx in range(0, len(cexp)):
            self.assertComplexNear(c1[idx], cexp[idx], " Complex numbers are different At index " + str(idx))
