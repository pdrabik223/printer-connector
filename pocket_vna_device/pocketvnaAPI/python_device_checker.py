...
class Step3OpenOpenCheck(WizardStep):
    ...
    def is_requried_standard(self, p1, p2, dist, standard):
        ...
        if p1 == p2:
            return dist < helper.adjust_threshold(helper.GoodDistanceThreshold) and (
                standard == "open" or standard == "open-open"
            )
        else:
            return dist < helper.adjust_threshold(helper.GoodDistanceThreshold) and (
                "open" in standard
    ...
    def bg_show_network_parameter(
        ...
        a.plot(
            standardNetwork.f,
            helper.db(standardNetwork.s[:, p1, p2]),
            label="Predefined Standard",
            color="gray",
            lw=3,
            linestyle="dashdot",
        )
        ...
        a.plot(
            scannedNetwork.f,
            helper.db(scannedNetwork.s[:, p1, p2]),
            label="Measured",
            color=curveColor,
            lw=curveWidth,
            linestyle="solid",
    ...
    def __init__(self, parent, ntwk, on_next_SL_test, on_rescan_requested):
        ...
        for p1 in range(0, 2):
            for p2 in range(0, 2):
                ...
                cvar1.set(0)
                self.scan_result_s[p1][p2] = tk.Checkbutton(
                    self.scan_result,
                    text="s{}{}".format(p1 + 1, p2 + 1),
                    variable=cvar1,
                )
...
class MyApp(tk.Frame):
    ...
    def next_select_step(self):
        ...
        if self.page_index == 0:
            ...
            self.current_wizard_page = Step2Connected(
                self,
                self.driver,
                self.on_open_open_network_scanned,
                self.on_connection_lost,
            )
        elif self.page_index == 2:
            self.current_wizard_page = Step3OpenOpenCheck(
                self, self.open_open_network, self.on_next_SL_test, self.jump_2_previous
            )
        elif self.page_index == 3:
            self.current_wizard_page = Step4Connected(
                self,
                self.driver,
                self.on_short_load_network_scanned,
                self.on_connection_lost,
            )
        elif self.page_index == 4:
            self.current_wizard_page = Step5ShortLoadCheck(
                self,
                self.short_load_network,
                self.on_test_complete,
                self.jump_2_previous,
            )
        else:
            self.page_index, self.page_code = -1, "nopage"
            self.next_select_step()
    ...
    def set_appropriate_connection(self, driver):
        ...
        for iface in list(
            [
                pocketvna.ConnectionInterfaceCode.CIface_VCI,
                pocketvna.ConnectionInterfaceCode.CIface_HID,
            ]
        ):
